...
def generate_mask(mask_rcnn_pth, mask_rcnn_cfg, image) -> np:
    """
    ...
    Parameters:
        ...
        image: input image
    """
    with open(mask_rcnn_cfg, 'rb') as fp:
        cfg = pickle.load(fp)
    
    cfg.MODEL.WEIGHTS = mask_rcnn_pth
    ...
    height, widht = mask.shape
    mask_img = np.zeros((height, widht), dtype=np.uint8)
    mask_pos = []
    ...
    mask_pos = sorted(mask_pos, key=lambda x: (x[0], x[1]))
    mask_pos = mask_pos[int(len(mask_pos)/2)]

    left = mask_pos[1]
    right = widht - mask_pos[1]
    up = mask_pos[0]
    down = height - mask_pos[0]

    max_radius = min(left, right, up, down)
    radius = np.random.randint(max_radius, max_radius+50)
    new_mask_img = np.zeros((height, widht), dtype=np.uint8)
    
    ### TODO: generate mask 
    new_mask_img = cv2.rectangle(new_mask_img, (mask_pos[1] - 100, mask_pos[0] - 100), (mask_pos[1] + max_radius - 50, mask_pos[0] + max_radius - 50), (255, 255, 255), -1)
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                ### TODO: add mask
                mask = generate_mask(opt.mask_rcnn_pth, opt.mask_rcnn_cfg, opt.image_path)
                ...
                tmp_en = z_inpaint.clone().cpu()

                for i in range(tmp_en.shape[0]):
                    tmp_img = tmp_en[i].permute(1,2,0).cpu().numpy()
                    tmp_img = Image.fromarray(tmp_img.astype(np.uint8))
                    tmp_img.save(os.path.join(sample_path, filename[:-4]+'_'+str(opt.seed)+"_en.png"))
                ...
                tmp_en = samples_ddim.detach().clone().cpu()
                for i in range(tmp_en.shape[0]):
                    tmp_img = tmp_en[i].permute(1,2,0).cpu().numpy()
                    tmp_img = Image.fromarray(tmp_img.astype(np.uint8))
                    tmp_img.save(os.path.join(sample_path, filename[:-4]+'_'+str(opt.seed)+"_en.png"))
